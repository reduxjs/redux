...
However, if you wrap [`createStore`](createStore.md) with [`applyMiddleware`](applyMiddleware.md), the middleware can interpret actions differently, and provide support for dispatching intents. Intents are usually asynchronous primitives like Promises, Observables, or thunks.
