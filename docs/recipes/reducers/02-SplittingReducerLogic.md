...
- ***case function***: a function is being used to handle the update logic for a specific action.  This may actually be a reducer function, or it may require other parameters to do its work properly.
- ***business function***: any other function that is used as part of the reducer logic, and is related to application-specific concepts
- ***utility function***: any other function that is used as part of the reducer logic, but is _not_ related to application-specific concepts and could easily be reused in other contexts
...
The term "*sub-reducer*" has also been used in various discussions to mean any function that is not the root reducer, although the term is not very precise.  
