...
Like with the earlier example, we have a function component called `Counter`, that stores some data in a `useState` hook.
...
We don't have to _just_ use selectors that have already been exported, either. For example, we could write:
...
Similarly, we know that if we had access to a Redux store, we could dispatch actions using action creators, like `store.dispatch(increent())`. Since we don't have access to the store itself, we need some way to have access to just the `dispatch` method.
