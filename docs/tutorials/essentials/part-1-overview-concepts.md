...
Let's start by looking at a small React counter component. It tracks a number in the component state and increments the number when a button is clicked:
...
- The **actions**: the events that occur in the app based on user input and that trigger updates in the state
...
However, this simplicity can break down when we have **multiple components that need to share and use the same state**. This is especially true if those components are located in different parts of the application. Sometimes this can be solved by ["lifting state up"](https://reactjs.org/docs/lifting-state-up.html) to parent components, but that doesn't always help.

One way to solve this is to extract the shared state from the components and put it into a centralized location outside of the component tree. With this, our component tree becomes a big "view" and any component can access the state or trigger actions (no matter where they are in the tree)!
...
// Alternatively, we can make a copy of the original array
...
- Check to see if the reducer cares about this action.
  - If so, make a copy of the state, update the copy with new values and then return it.
...
Notice that this `addNumbers` "reduce callback" function doesn't need to keep track of anything itself. It takes the `previousResult` and `currentItem` arguments, does something with them, and then returns a new result value.

**A Redux reducer function is exactly the same idea as this "reduce callback" function!** It takes a "previous result" (the `state`) and the "current item" (the `action` object), decides a new state value based on those arguments, and then returns that new state.
...
- **Redux uses several types of code**
  - _Actions_ are plain objects with a `type` field that describe "what happened" in the app
