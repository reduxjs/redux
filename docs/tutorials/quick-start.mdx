...
In [Part 2](./part-2.md), we'll use that knowledge to build a small blogging app with some real-world features.
...
The project has been created using [the official Redux template for Create-React-App](https://github.com/reduxjs/cra-template-redux), and uses two important package:
...
The Redux store is created using the `configureStore` function from Redux Toolkit. `configureStore` requires that we pass in a `reducer` argument, which can be either a function, or an object.

Earlier, we saw that we can pass a reducer function directly as the `reducer` argument:
