...
**_All_ of that code can be replaced with [Redux Toolkit's "RTK Query" data fetching and caching layer](/toolkit/rtk-query/overview)!**
...
import { pingSaga } from '../sagas/ping'
...
const pingSlice = createSlice({
  ...
  initialState,
  reducers: {
    // "Signal" action, dispatched only to kick off the listener
    ping() {},
    pong(state, action) {
      // state update here
    }
...
Per [our standard TypeScript setup and usage guidelines](../tutorials/quick-start.md), we specifically teach setting up "pre-typed" aliases for the hooks, so that those have the correct types baked in, and only use those pre-typed hooks in the app.
...
- **Tutorials**
  - [Redux Essentials: Redux Toolkit App Structure](../tutorials/essentials/part-2-app-structure.md)
  - [Redux Fundamentals: Modern Redux with Redux Toolkit](../tutorials/fundamentals/part-8-modern-redux.md)
  - [Redux Quick Start](../tutorials/quick-start.md)
- **Additional Documentation**
  ...
  - [Redux Toolkit: Usage with TypeScript](/toolkit/usage/usage-with-typescript)
- **Articles**
  - [Presentation: Modern Redux with Redux Toolkit](https://blog.isquaredsoftware.com/2022/06/presentations-modern-redux-rtk/)
