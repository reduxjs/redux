...
We have traditionally recommended thunks as the default approach, and Redux Toolkit specifically includes the [`createAsyncThunk` API](/toolkit/api/createAsyncThunk) for the "request and dispatch" use case. For other use cases, you can write your own thunk functions.
...
- 👎: Not _quite_ as flexible as sagas/observables
...
Redux Toolkit includes [RTK Query](/toolkit/rtk-query/overview), a purpose-built data fetching and caching solution for Redux apps. It's designed to simplify common cases for loading data in a web application, eliminating the need to hand-write data fetching & caching logic yourself.
...
- Docs and Tutorials:
  ...
  - [Redux Toolkit: RTK Query Overview](/toolkit/rtk-query/overview)
