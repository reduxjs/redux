...
With the arrival of [the React-Redux hooks API](/react-redux/api/hooks), that situation has changed. The community has switched away from the "container/presentational" pattern in general, and [components now have access to `dispatch` directly via the `useDispatch` hook](../tutorials/fundamentals/part-5-ui-and-react.md#dispatching-actions-with-usedispatch). This _does_ mean that it's possible to have more logic directly inside of a component, such as an async fetch + dispatch of the results. However, thunks have access to `getState`, which components do not, and there's still value in moving that logic outside of components.
...
Thunks are "one-shot" functions, with no sense of a lifecycle. They also cannot see other dispatched actions. So, they should not generally be used for initializing persistent connections like websockets, and you can't use them to respond to other actions. For those cases, use Redux Toolkit's [listener middleware](/toolkit/api/createListenerMiddleware), which runs logic in response to dispatched actions or state changes. See [Side Effects Approaches](./side-effects-approaches.mdx) for a comparison.
...
Redux Toolkit's `configureStore` supports [this as part of its middleware customization in `getDefaultMiddleware`](/toolkit/api/getDefaultMiddleware):
...
The default `Dispatch` type from the Redux core doesn't know about thunks, so `dispatch(someThunk())` is a type error unless the dispatch type includes the thunk middleware. The usual setup is to derive `AppDispatch` from the store, define an `AppThunk` type for hand-written thunks, and use a pre-typed `useAppDispatch` hook in components:

```ts title="Typing hand-written thunks" no-transpile
// app/store.ts
...
export const addTodosIfAllowed =
  (todoText: string): AppThunk =>
  (dispatch, getState) => {
    if (selectCanAddNewTodo(getState(), MAX_TODOS)) {
      dispatch(todoAdded(todoText))
    }
  }
...
dispatch(addTodosIfAllowed('Buy milk'))
```

`configureStore` infers the dispatch type from its middleware, so `AppDispatch` already includes the thunk middleware's overload. See [Usage with TypeScript](./UsageWithTypescript.md#define-root-state-and-dispatch-types) for the full setup.
...
Redux Toolkit includes the [**RTK Query data fetching API**](/toolkit/rtk-query/overview). RTK Query is a purpose built data fetching and caching solution for Redux apps, and **can eliminate the need to write _any_ thunks or reducers to manage data fetching**.
...
```js title="RTK Query: API slice (services/pokemon.js)"
