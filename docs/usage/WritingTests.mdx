...
Our recommendation is to mock async requests at the `fetch/xhr` level using [Mock Service Worker (`msw`)](https://mswjs.io/) or a similar tool. By mocking requests at this level, none of the thunk logic has to change in a test - the thunk still tries to make a "real" async request, it just gets intercepted. See the ["Integration Test" example](#writing-integration-tests-with-components) for an example of testing a component which internally includes the behavior of a thunk.

:::info

If you prefer, or are otherwise required to write unit tests for your action creators or thunks, refer to the tests that Redux Toolkit uses for [`createAction`](https://github.com/reduxjs/redux-toolkit/blob/master/packages/toolkit/src/tests/createAction.test.ts) and [`createAsyncThunk`](https://github.com/reduxjs/redux-toolkit/blob/master/packages/toolkit/src/tests/createAsyncThunk.test.ts).
...
We need to create fake `getState`, `dispatch`, and `next` functions. We use `vi.fn()` to create stubs; with Jest, the equivalent is `jest.fn()`.

The `invoke` function runs our middleware in the same way Redux does.

```ts title="thunkMiddleware.test.ts"
// file: thunkMiddleware.ts noEmit
import type { Middleware } from '@reduxjs/toolkit'
export const thunkMiddleware: Middleware =
  ({ dispatch, getState }) =>
  next =>
...
const create = () => {
  ...
  const invoke = (action: unknown) =>
    thunkMiddleware(store)(next)(action as UnknownAction)
...
```ts title="thunkMiddleware.test.ts" no-transpile
...
test('passes dispatch and getState', () => {
  ...
  invoke((dispatch: Dispatch, getState: () => unknown) => {
