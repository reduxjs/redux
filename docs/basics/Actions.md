...
We'll add one more action type to describe a user ticking off a todo as completed. We refer to a particular todo by `index` because we store them in an array. In a real app, it would be wiser to generate a unique ID every time a new todo is created.
