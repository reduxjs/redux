...
* [Why should I use an immutable-focused library such as Immutable.JS?](usingimmutablejs.md#why-use-immutable-library)
* [Why should I choose Immutable.JS as an immutable library?](usingimmutablejs.md#why-choose-immutable-js)
* [What are the issues with using Immutable.JS?](usingimmutablejs.md#issues-with-immutable-js)
* [Is Immutable.JS worth the effort?](usingimmutablejs.md#is-immutable-js-worth-effort)
* [What are some opinionated Best Practices for using Immutable.JS with Redux?](usingimmutablejs.md#immutable-js-best-practices)
...
This issue can be avoided by [uncoupling your application logic from your data structures](https://medium.com/@dtinth/immutable-js-persistent-data-structures-and-structural-sharing-6d163fbd73d2#.z1g1ofrsi), as outlined in the [best practices section](usingimmutablejs.md#immutable-js-best-practices) below.
...
This can be prevented by using `toJS()` in a Higher Order Component, as discussed in the [Best Practices section](usingimmutablejs.md#immutable-js-best-practices) below.
...
* [FAQ: Immutable Data](https://github.com/reduxjs/redux/tree/7614c9a52bc8858294693ef1d9cc214533b608fe/faq/ImmutableData.html#immutability-issues-with-react-redux)
...
_Note: if your app requires high performance, you may need to avoid_ `toJS()` _altogether, and so will have to use Immutable.JS in your dumb components. However, for most apps this will not be the case, and the benefits of keeping Immutable.JS out of your dumb components \(maintainability, portability and easier testing\) will far outweigh any perceived performance improvements of keeping it in._

_In addition, using_ `toJS` _in a Higher Order Component should not cause much, if any, performance degradation, as the component will only be called when the connected component’s props change. As with any performance issue, conduct performance checks first before deciding what to optimize._
