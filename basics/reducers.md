...
We'll explore how to perform side effects in the [advanced walkthrough](../advanced/). For now, just remember that the reducer must be pure. **Given the same arguments, it should calculate the next state and return it. No surprises. No side effects. No API calls. No mutations. Just a calculation.**
